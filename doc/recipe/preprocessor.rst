...
consistent.
This makes it possible to search for fx files under multiple ensemble members
or experiments, for example, ``ensemble: '*'`` or ``exp: '*'``.
Note that the ``*`` character must be quoted since ``*`` is a special charcter
in YAML.
This functionality is only supported for time-invariant fx variables (i.e.
frequency ``fx``).
Note also that if multiple directories of matching fx files are found,
ESMValTool will prioritize the ensemble ``r0i0p0`` (if it exists), otherwise
