...
class DerivedVariable(DerivedVariableBase):
    ...
    @staticmethod
    def calculate(cubes):
        """Compute total cloud water path.

        Note
        ----
        Some datasets output the variable `clwvi` which only contains `lwp`. In
        these cases, the sum of the `clwvi` and `clivi` cubes is returned.
        ...
        if (project in affected_projects and dataset in bad_datasets):
            ...
            tcwp_cube = clwvi_cube + clivi_cube
        else:
            tcwp_cube = clwvi_cube
