...
def fix_vertical_level(cube):
    if cube.coords()[1].var_name in ['lev','deptht']:
        try:
            cube.coord("generic").rename("depth")
        except iris.exceptions.CoordinateNotFoundError:
            try:
                cube.coord("Vertical T levels").rename("depth")
            except iris.exceptions.CoordinateNotFoundError:
                try:
                    cube.coord("model_level_number").rename("depth")
                except iris.exceptions.CoordinateNotFoundError:
                    raise iris.exceptions.CoordinateNotFoundError(
                        "None of the expected coordinate names ('generic','model_level_number','Vertical T levels') were found in the cube."
                    )
...
def fix_variable_info(self, cube):
    ...
    source_file = cube.attributes['source_file']
    cube.attributes = {}
    version = 'v1.0'

    cube.attributes["history"] = (
            f"File generated from {source_file} using the "
            "https://earth.bsc.es/gitlab/es/es-esmvaltool/-/blob/master/cmorizers/oras5/oras5.py "
            f"cmorizer script version {version}."
        )

    return cube

def fix_mask(self, cube):
    if self.vardef.short_name not in ['areacello',"volcello"]:
        ...
        mask_path=os.path.join(os.path.dirname(__file__), f"{name}.npy")
        mask = np.load(mask_path)
        mask = np.expand_dims(mask, 0)
        masked_data = da.ma.masked_where(mask, cube.lazy_data())
...
class AllVars(NativeDatasetFix):
    ...
    def fix_metadata(self, cube):
        ...
        if self.vardef.short_name in ['areacello',"volcello"]:
            cube.remove_coord('time')
