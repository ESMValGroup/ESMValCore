...
This module provides the function :py:func:`esmvalcore.io.esgf.find_files`
for searching for files on ESGF using the ESMValTool vocabulary.
It returns :class:`esmvalcore.io.esgf.ESGFFile` objects, which have a convenient
:meth:`esmvalcore.io.esgf.ESGFFile.download` method for downloading the file.
A :func:`esmvalcore.io.esgf.download` function for downloading multiple files in
parallel is also available.

It also provides an :class:`esmvalcore.io.esgf.ESGFDataSource` that can be
...
from esmvalcore.io.esgf._download import ESGFFile, download
