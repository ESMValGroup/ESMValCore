...
def cubes_bounds():
    ...
    correct_lon_coord = iris.coords.DimCoord(
        [0, 356.25],
        bounds=[[-1.875, 1.875], [354.375, 358.125]],
        var_name="lon",
        standard_name="longitude",
        circular=True,
    )
    wrong_lon_coord = iris.coords.DimCoord(
        [0, 356.25],
        bounds=[[-1.875, 1.875], [354.375, 360]],
        var_name="lon",
        standard_name="longitude",
    )
    correct_cube = iris.cube.Cube(
...
def test_get_uas_fix():
    ...
    assert fix == [Uas(None), AllVars(None), GenericFix(None)]
...
def thetao_cubes():
    ...
    lon_coord = iris.coords.DimCoord(
        [-0.9375, 357.1875],
        bounds=[[-1.875, 0.0], [356.25, 358.125]],
        var_name="lon",
        standard_name="longitude",
    )
