...
ALT_COORD = iris.coords.AuxCoord(
    [0.0],
    bounds=[[-100.0, 500.0]],
...
ALT_COORD_NB = iris.coords.AuxCoord(
    [0.0],
    standard_name="altitude",
    units="m",
    var_name="alt",
...
ALT_COORD_KM = iris.coords.AuxCoord(
    [0.0],
    bounds=[[-0.1, 0.5]],
    var_name="alt",
    long_name="altitude",
...
P_COORD_NB = iris.coords.AuxCoord(
    [101325.0],
    standard_name="air_pressure",
    units="Pa",
    var_name="plev",
...
def test_add_plev_from_altitude(cube, output):
    ...
    if output is None:
        ...
        msg = (
            "Cannot add 'air_pressure' coordinate, 'altitude' coordinate "
            "not available"
        )
...
P_COORD_HPA = iris.coords.AuxCoord(
    [1013.25],
...
def test_add_altitude_from_plev(cube, output):
    ...
    if output is None:
        ...
        msg = (
            "Cannot add 'altitude' coordinate, 'air_pressure' coordinate "
            "not available"
        )
        assert str(err.value) == msg
        return
