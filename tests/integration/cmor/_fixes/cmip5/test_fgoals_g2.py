...
@unittest.mock.patch(
    'esmvalcore.cmor._fixes.cmip5.fgoals_g2.add_pressure_level_coordinate',
    autospec=True)
def test_cl_fix_metadata(mock_add_pressure_level):
    """Test ``fix_metadata`` for ``cl``."""
    cubes = CubeList([Cube(0.0, var_name='cl'), Cube(1.0, var_name='x')])
    fix = Cl(None)
    fixed_cubes = fix.fix_metadata(cubes)
    mock_add_pressure_level.assert_called_once_with(cubes[0])
