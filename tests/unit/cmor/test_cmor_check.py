...
class TestCMORCheck(unittest.TestCase):
    ...
    def test_time_units_automatic_fix_wrong_format(self):
        """Test automatic fix for time units."""
        self.cube.coord('time').units = 'days since 1860-1-1 00:00:00'
        ...
        assert (self.cube.coord('time').units.origin ==
                'days since 1850-1-1 00:00:00')
        assert self.cube.attributes['parent_time_units'] == (
            'days since 1850-1-1 00:00:00')
        assert self.cube.attributes['branch_time_in_parent'] == 0.0
        assert self.cube.attributes['branch_time_in_child'] == 3652.0
