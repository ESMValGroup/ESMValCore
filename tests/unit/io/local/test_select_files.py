...
from esmvalcore.io.local import LocalFile, _select_files

if TYPE_CHECKING:
    from collections.abc import Sequence


def to_local_files(filenames: Sequence[str]) -> Sequence[LocalFile]:
    """Convert a list of filenames to LocalFile objects."""
    result = []
    for filename in filenames:
        file = LocalFile(filename)
        file.facets["timerange"] = file.stem.split("_")[-1].replace("-", "/")
        result.append(file)
    return result
...
def test_select_files_monthly_resolution() -> None:
...
def test_select_files_daily_resolution() -> None:
...
def test_select_files_sub_daily_resolution() -> None:
    ...
    files_separator = to_local_files(
        [
    ...
    expected_no_separator = to_local_files(
        [
            filename + "196011010900-196110312100.nc",
        ],
    )

    expected_separator = to_local_files(
        [
            filename + "19601101T0900-19611031T2100.nc",
        ],
    )
...
def test_select_files_time_period() -> None:
    ...
    expected_datetime = to_local_files(
        [
            filename_datetime + "196011010900-196110312100.nc",
            filename_datetime + "196111010900-196210312100.nc",
        ],
...
def test_select_files_varying_format() -> None:
