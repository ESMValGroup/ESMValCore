...
def cubes():
    ...
    rlds_cube = get_cube(
        [[[100.0]]], air_pressure_coord=False, standard_name=rlds_name
    )
    rlds_cube.attributes["positive"] = "down"
    rlns_cube = get_cube(
        [[[50.0]]], air_pressure_coord=False, standard_name=rlns_name
    )
...
def test_rlntcs_calculation(cubes):
    ...
    np.testing.assert_allclose(out_cube.data, np.array([[[50.0]]]))
