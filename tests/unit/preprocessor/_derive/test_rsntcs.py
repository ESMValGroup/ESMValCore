...
def cubes():
    ...
    rsdt_cube = iris.cube.Cube(
        [[1.0, 2.0], [0.0, -2.0]], standard_name=rsdt_name
    )
    rsutcs_cube = iris.cube.Cube(
        [[5.0, -1.2], [0.8, -3.0]], standard_name=rsutcs_name
    )
...
def test_rsntcs_calculation(cubes):
    ...
    np.testing.assert_allclose(
        out_cube.data, np.array([[-4.0, 3.2], [-0.8, 1.0]])
    )
