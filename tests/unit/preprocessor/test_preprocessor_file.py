...
def test_save_no_cleanup(mocker):
    """Test ``save``."""
    mocker.patch.object(esmvalcore.preprocessor, 'preprocess', autospec=True)
    mocker.patch.object(esmvalcore.preprocessor, 'save', autospec=True)
    product = mocker.create_autospec(PreprocessorFile, instance=True)
    product.settings = {'save': {'filename': Path('file1.nc')}}
    product._cubes = [mocker.sentinel.cube]
    product._input_files = mocker.sentinel.input_files

    PreprocessorFile.save(product)

    esmvalcore.preprocessor.save.assert_called_with(
        [mocker.sentinel.cube],
        filename=Path('file1.nc'),
    )
    esmvalcore.preprocessor.preprocess.assert_not_called()


def test_save_cleanup(mocker):
    """Test ``save``."""
    mocker.patch.object(esmvalcore.preprocessor, 'preprocess', autospec=True)
    mocker.patch.object(esmvalcore.preprocessor, 'save', autospec=True)
    product = mocker.create_autospec(PreprocessorFile, instance=True)
    product.settings = {'save': {'filename': Path('file1.nc')}, 'cleanup': {}}
    product._cubes = [mocker.sentinel.cube]
    product._input_files = mocker.sentinel.input_files
    ...
    esmvalcore.preprocessor.preprocess.assert_called_with(
        [Path('file1.nc')],
        'cleanup',
