...
class Test(tests.Test):
    ...
    def setUp(self):
        ...
        scoord = iris.coords.DimCoord(
            [36.0, 36.5, 37.0],
            long_name="ocean_sigma_coordinate",
            bounds=[[35.5, 36.25], [36.25, 36.75], [36.75, 37.5]],
            units="kg m-3",
            attributes={"positive": "down"},
        )
        zcoord_nobounds = iris.coords.DimCoord(
            [0.5, 5.0, 50.0],
            long_name="zcoord",
            units="m",
            attributes={"positive": "down"},
        ...
        zcoord_4d = iris.coords.AuxCoord(
            np.broadcast_to([[[[0.5]], [[5.0]], [[50.0]]]], (2, 3, 2, 2)),
            long_name="zcoord",
            bounds=np.broadcast_to(
                [[[[[0.0, 2.5]]], [[[2.5, 25.0]]], [[[25.0, 250.0]]]]],
                (2, 3, 2, 2, 2),
            ),
            units="m",
            attributes={"positive": "down"},
        ...
        zcoord_3d_invalid_bounds = iris.coords.AuxCoord(
            np.broadcast_to([[[0.5]], [[5.0]], [[50.0]]], (3, 2, 2)),
            long_name="zcoord",
            bounds=np.broadcast_to(
                [
                    [[[0.0, 2.5, 2.5, 3.0]]],
                    [[[2.5, 25.0, 25.0, 30.0]]],
                    [[[25.0, 250.0, 250.0, 300.0]]],
                ],
                (3, 2, 2, 4),
            ),
            units="m",
        ...
        lons2d = iris.coords.AuxCoord(
            [[1.5, 2.5], [1.2, 2.7]],
            standard_name="longitude",
            units="degrees_east",
            coord_system=coord_sys,
        )
        lats2d = iris.coords.AuxCoord(
            [[1.5, 2.5], [1.2, 2.7]],
            standard_name="latitude",
            units="degrees_north",
            coord_system=coord_sys,
    ...
    def test_extract_volume_intervals(self):
        ...
        open_interval = extract_volume(self.grid_3d, 0.0, 5.0)
    ...
    def test_extract_volume_mixed_intervals(self):
        left_closed = extract_volume(self.grid_3d, 0.5, 5.0, "left_closed")
    ...
    def test_extract_volume_nearest_values(self):
        ...
        expected_levels_default = np.array([0.5, 5.0])
    ...
    def test_extract_volume_error(self):
        with self.assertRaises(ValueError) as err:
            extract_volume(self.grid_3d, 0.0, 5.0, "wrong")
        self.assertEqual(
            (
    ...
    def test_extract_volume_mean(self):
        ...
        expected_mean = np.ma.array([1.0, 1.0], mask=False)
    ...
    def test_volume_statistics_long(self):
        ...
        expected = np.ma.array([1.0, 1.0, 1.0, 1.0], mask=False)
        self.assert_array_equal(result.data, expected)
        self.assertEqual(result.units, "kg m-3")
    ...
    def test_extract_surface_from_atm(self):
        ...
        # Create surface air pressure ancillary variable for interpolation
